...
class ControllerWaiter(ProcessWaiter):
    def __init__(self, listener_ip, listener_port, completition_event):
        ...
        log(DEBUG, "[Listening socket is now open]")
    ...
    def start(self):
        try:
            ...
            log(DEBUG, "[Controller connected:" + str(client_addr) + "]")
            self._success = True

        except Exception as e:
            log(ERROR, "[Failed to obtain connection {}]".format(e))
...
class FeedbackWaiter(ProcessWaiter):
    def __init__(self, feedback_listener, event_obj):
    ...
    def start(self):
        try:
            ...
            log(ERROR, "Failed to get controller message:{}".format(e))
...
class TestBoilerplate:
    ...
    def setup(self):
        ...
        self.network = create_network(proxy_ip="192.168.1.248", proxy_port=6833)
        start_network(self.network)

        # Start feedback waiter
        self.feedback_ready_event = Event()
        self.message_waiter = FeedbackWaiter(self._feedback_listener, self.feedback_ready_event)
        ...
        wait_thread = Thread(target=self.message_waiter.start)
        wait_thread.setDaemon(True)
        wait_thread.start()
    ...
    def wait_feedback_msg(self, timeout=_CONNECTION_TIMEOUT):
